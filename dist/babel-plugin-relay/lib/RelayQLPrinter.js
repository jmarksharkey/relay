...
module.exports = function (t, options) {
  ...
  var RelayQLPrinter = function () {
    ...
    RelayQLPrinter.prototype.print = function print(definition, substitutions) {
      ...
      if (definition instanceof RelayQLQuery) {
        ...
        throw new (require('././RelayTransformError'))(require('util').format('Unsupported definition: %s', definition), definition.getLocation());
      }
    ...
    RelayQLPrinter.prototype.printQuery = function printQuery(query, enableValidation) {
      ...
      if (rootFields.length !== 1 && enableValidation) {
        throw new (require('././RelayTransformError'))(require('util').format('There are %d fields supplied to the query named `%s`, but queries ' + 'must have exactly one field.', rootFields.length, query.getName()), query.getLocation());
      }
      ...
      if (rootFieldArgs.length > 1) {
        throw new (require('././RelayTransformError'))(require('util').format('Invalid root field `%s`; Relay only supports root fields with zero ' + 'or one argument.', rootField.getName()), query.getLocation());
      }
    ...
    RelayQLPrinter.prototype.printFragment = function printFragment(fragment) {
      ...
      if (selectVariables) {
        ...
        if (!Array.isArray(selectVariablesValue)) {
          throw new (require('././RelayTransformError'))('The variables argument to the @relay directive should be an array ' + 'of strings.', fragment.getLocation());
        }
    ...
    RelayQLPrinter.prototype.printMutation = function printMutation(mutation, enableValidation) {
      ...
      if (rootFields.length !== 1 && enableValidation) {
        throw new (require('././RelayTransformError'))(require('util').format('There are %d fields supplied to the mutation named `%s`, but ' + 'mutations must have exactly one field.', rootFields.length, mutation.getName()), mutation.getLocation());
      }
      var rootField = rootFields[0];
      var rootFieldType = rootField.getType();
    ...
    RelayQLPrinter.prototype.printSubscription = function printSubscription(subscription, enableValidation) {
      ...
      if (rootFields.length !== 1 && enableValidation) {
        throw new (require('././RelayTransformError'))(require('util').format('There are %d fields supplied to the subscription named `%s`, but ' + 'subscriptions must have exactly one field.', rootFields.length, subscription.getName()), subscription.getLocation());
      }
      var rootField = rootFields[0];
      var rootFieldType = rootField.getType();
      validateMutationField(rootField);
      ...
      parent.getSelections().forEach(function (selection) {
        if (selection instanceof RelayQLFragmentSpread) {
          ...
          if (selection.getDirectives().length !== 0) {
            throw new (require('././RelayTransformError'))('Directives are not yet supported for `${fragment}`-style fragment ' + 'references.', selection.getLocation());
          }
    ...
    maybeMetadata) {
      ...
      if (relayDirective) {
        relayDirective.getArguments().forEach(function (arg) {
          if (arg.isVariable()) {
            throw new (require('././RelayTransformError'))(require('util').format('You supplied `$%s` as the `%s` argument to the `@relay` ' + 'directive, but `@relay` require scalar argument values.', arg.getVariableName(), arg.getName()), node.getLocation());
          }
  ...
  function validateField(field, parentType) {
    if (field.getName() === 'node') {
      ...
      if (!parentType.isQueryType() && argNames.length === 1 && argNames[0] === ID) {
        throw new (require('././RelayTransformError'))(require('util').format('You defined a `node(%s: %s)` field on type `%s`, but Relay requires ' + 'the `node` field to be defined on the root type. See the Object ' + 'Identification Guide: \n' + 'http://facebook.github.io/relay/docs/graphql-object-identification.html', ID, argNames[0] && argTypes[argNames[0]].getName({ modifiers: true }), parentType.getName({ modifiers: false })), field.getLocation());
      }
    }
  }
  ...
  function validateConnectionField(field) {
    ...
    if (!condition) {
      throw new (require('././RelayTransformError'))(require('util').format('Connection arguments `%s(first: <count>, last: <count>)` are ' + 'not supported unless both are variables. Use `(first: <count>)`, ' + '`(last: <count>)`, or `(first: $<var>, last: $<var>)`.', field.getName()), field.getLocation());
    }
    condition = !first || !before || first.isVariable() && before.isVariable();
    if (!condition) {
      throw new (require('././RelayTransformError'))(require('util').format('Connection arguments `%s(before: <cursor>, first: <count>)` are ' + 'not supported unless both are variables. Use `(first: <count>)`, ' + '`(after: <cursor>, first: <count>)`, `(before: <cursor>, last: <count>)`, ' + 'or `(before: $<var>, first: $<var>)`.', field.getName()), field.getLocation());
    }
    condition = !last || !after || last.isVariable() && after.isVariable();
    if (!condition) {
      throw new (require('././RelayTransformError'))(require('util').format('Connection arguments `%s(after: <cursor>, last: <count>)` are ' + 'not supported unless both are variables. Use `(last: <count>)`, ' + '`(before: <cursor>, last: <count>)`, `(after: <cursor>, first: <count>)`, ' + 'or `(after: $<var>, last: $<var>)`.', field.getName()), field.getLocation());
    }
    ...
    forEachRecursiveField(field, function (subfield) {
      if (subfield.getName() === FIELDS.edges || subfield.getName() === FIELDS.pageInfo) {
        ...
        if (!hasCondition) {
          throw new (require('././RelayTransformError'))(require('util').format('You supplied the `%s` field on a connection named `%s`, but you did ' + 'not supply an argument necessary to do so. Use either the `find`, ' + '`first`, or `last` argument.', subfield.getName(), field.getName()), field.getLocation());
        }
      } else {
  ...
  function validateMutationField(rootField) {
    ...
    if (declaredArgNames.length !== 1) {
      throw new (require('././RelayTransformError'))(require('util').format('Your schema defines a mutation field `%s` that takes %d arguments, ' + 'but mutation fields must have exactly one argument named `%s`.', rootField.getName(), declaredArgNames.length, INPUT_ARGUMENT_NAME), rootField.getLocation());
    }

    if (declaredArgNames[0] !== INPUT_ARGUMENT_NAME) {
      throw new (require('././RelayTransformError'))(require('util').format('Your schema defines a mutation field `%s` that takes an argument ' + 'named `%s`, but mutation fields must have exactly one argument ' + 'named `%s`.', rootField.getName(), declaredArgNames[0], INPUT_ARGUMENT_NAME), rootField.getLocation());
    }

    var rootFieldArgs = rootField.getArguments();
    if (rootFieldArgs.length > 1) {
      throw new (require('././RelayTransformError'))(require('util').format('There are %d arguments supplied to the mutation field named `%s`, ' + 'but mutation fields must have exactly one `%s` argument.', rootFieldArgs.length, rootField.getName(), INPUT_ARGUMENT_NAME), rootField.getLocation());
    }
  }
