...
var getArgumentValues = require('./RelayStoreUtils').getArgumentValues,
...
var RelayResponseNormalizer = function () {
  ...
  RelayResponseNormalizer.prototype._validateRecordType = function _validateRecordType(record, field, payload) {
    ...
    require('fbjs/lib/warning')(require('./RelayModernRecord').getType(record) === typeName, 'RelayResponseNormalizer: Invalid record `%s`. Expected %s to be ' + 'be consistent, but the record was assigned conflicting types ' + '`%s` and `%s`.', require('./RelayModernRecord').getDataID(record), TYPENAME_KEY, require('./RelayModernRecord').getType(record), typeName);
  };
